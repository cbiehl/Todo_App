...
public class CustomCursorAdapter extends RecyclerView.Adapter<CustomCursorAdapter.TaskViewHolder> {
    ...
    public void onBindViewHolder(TaskViewHolder holder, int position) {
        ...
        final String priorityString = "" + priority; // converts int to String
        holder.priorityView.setText(priorityString);

        GradientDrawable priorityCircle = (GradientDrawable) holder.priorityView.getBackground();
        // Get the appropriate background color based on the priority
        final int priorityColor = getPriorityColor(priority);
        priorityCircle.setColor(priorityColor);

        // Listen for clicks on single items and launch AddTrip Activity
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Pass the Tag (id) to the intent to display / edit a task entry
                Intent i = new Intent(v.getContext(), AddTaskActivity.class);
                i.putExtra(TaskContract.TaskEntry._ID, String.valueOf((int) v.getTag()));
                i.putExtra(TaskContract.TaskEntry.COLUMN_DESCRIPTION, description);
                i.putExtra(TaskContract.TaskEntry.COLUMN_PRIORITY, priority);
